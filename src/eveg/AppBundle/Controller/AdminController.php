...
//print($catminatCode);
	 ...
	 *
	 */
	public function importRepartitionDepFrAction()
	{
		
		set_time_limit(1000);
		ini_set('memory_limit', '1024M');

		$import = $this->csv_to_array($_SERVER['DOCUMENT_ROOT'].'/bvDepFr_18062015.csv', ';');
		$importKeys = array_keys($import);
		//return null;
		//$batchSize = 20;
		$em = $this->getDoctrine()->getManager();
		$entities = $em->getRepository('evegAppBundle:SyntaxonCore')->findAll();

		print(count($import));print(' -- ');print(count($entities));

		foreach ($entities as $key => $entity) {
			if($entity->getRepartitionDepFr() != null)
			{
				$em->remove($entity->getRepartitionDepFr());	
				$entity->setRepartitionDepFr(null);
			}			
		}
		$em->flush();
		$em->clear();
		...
		foreach ($entities as $key => $entity) {
			...
			if($importKey != null) {
				$newRepartition->set01($import[$importKey]['01'])
		...
		//return new JsonResponse(array('import' => $import));
		return null;
	}
	...
	public function csv_to_array($filename='', $delimiter=',')
	{
		if($this->container->get('kernel')->getEnvironment() == 'dev') {
			...
			Throw new HttpException(401, "The 'csv_to_array' function is only available in dev mode.", 1);
		}
